...
describe("resetPasswordHandler", () => {
  beforeEach(() => {
    ...
    vi.mocked(resetTokenRepository.deleteByToken).mockReturnValue(Effect.succeed(undefined as any))
  })
  ...
  it("returns 401 when consumeResetToken reports token already consumed (concurrent replay)", async () => {
    ...
    vi.mocked(consumeResetToken).mockReturnValue(
      Effect.fail(new ConflictError("token", "Reset token not found or already consumed"))
    )
    const res = await post(VALID_BODY)
    expect(res.status).toBe(401)
  })
