 ...
 */
export const verifyJwt = (token: string): Effect.Effect<User, JwtError> =>
  Effect.gen(function* () {
    ...
    if (headerObj["alg"] !== "EdDSA") {
      return yield* Effect.fail(
        new TokenInvalidError({ reason: "unexpected_algorithm" })
      )
    }
    ...
    if (!valid) {
      return yield* Effect.fail(
        new TokenInvalidError({ reason: "bad_signature" })
      )
    }
    ...
    if (!payload.sub || !payload.role || !payload.sessionId) {
      return yield* Effect.fail(
        new TokenInvalidError({ reason: "missing_claims" })
      )
    }
