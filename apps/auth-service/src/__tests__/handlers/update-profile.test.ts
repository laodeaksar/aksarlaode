...
const app = new Elysia().patch("/profile", updateProfileHandler, {
  body: UpdateProfileBody,
})

function patch(
  body: unknown,
  headers: Record<string, string> = USER_ID_HEADER
) {
  return app.handle(
...
describe("updateProfileHandler", () => {
  beforeEach(() => {
    vi.clearAllMocks()
    vi.mocked(userRepository.update).mockReturnValue(
      Effect.succeed(UPDATED_USER as any)
    )
  })
  ...
  it("returns 400 when avatarUrl points to an arbitrary external domain", async () => {
    const res = await patch({
      avatarUrl: "https://attacker.example.com/evil.png",
    })
    const body = await res.json()
    expect(res.status).toBe(400)
  ...
  it("returns 400 when avatarUrl targets an internal IP (SSRF attempt)", async () => {
    const res = await patch({
      avatarUrl: "https://169.254.169.254/latest/meta-data/",
    })
    expect(res.status).toBe(400)
  })
