...
describe("loginHandler", () => {
  beforeEach(() => {
    ...
    vi.mocked(sessionRepository.create).mockReturnValue(
  ...
  it("returns 429 and skips DB lookup when email is locked out", async () => {
    vi.mocked(recordEmailAttempt).mockResolvedValue({
      locked: true,
      retryAfterSec: 300,
    });
    const res = await post({
  ...
  it("does NOT include plaintext email in LOGIN_SUCCESS audit event", async () => {
    ...
    const successCall = vi
      ...
      .mock.calls.find(([entry]) => entry.event === "LOGIN_SUCCESS");
  ...
  it("emits LOGIN_FAILED audit event with masked email on bad credentials", async () => {
    ...
    const failCall = vi
      .mocked(writeAuditLog)
      .mock.calls.find(([entry]) => entry.event === "LOGIN_FAILED");
  ...
  it("transparently upgrades a legacy PBKDF2 hash to Argon2id on successful login", async () => {
    ...
    vi.mocked(hashPassword).mockReturnValue(
      Effect.succeed("$argon2id$v=19$new-hash")
    );
    vi.mocked(userRepository.updatePasswordHash).mockReturnValue(
  ...
  it("still returns 200 when the Argon2id upgrade fails (fail-open)", async () => {
    vi.mocked(needsRehash).mockReturnValue(true);
    vi.mocked(hashPassword).mockReturnValue(
      Effect.fail(new Error("hash error") as any)
    );
