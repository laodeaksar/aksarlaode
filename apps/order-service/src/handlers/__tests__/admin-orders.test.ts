...
const RAW_DOC = {
  ...
  statusHistory: [
    ...
    {
      _id: "664fabc0000000000000005",
      status: "__NOTE__",
      note: "Dicurigai fraud",
      changedBy: "admin-007",
      timestamp: now,
    },
    {
      _id: "664fabc0000000000000006",
      status: "PAID",
      note: null,
...
describe("adminListOrdersHandler — response shaping", () => {
  ...
  it("strips __NOTE__ entries from each item's statusHistory", async () => {
    const res = (await adminListOrdersHandler(makeCtx() as any)) as any;
    for (const item of res.items) {
      const hasNote = item.statusHistory.some(
        (e: any) => e.status === "__NOTE__"
      );
