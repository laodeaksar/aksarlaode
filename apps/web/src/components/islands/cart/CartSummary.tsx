...
export function CartSummary() {
  ...
  if (items.length === 0) {
    return (
      <div class="text-center py-16">
        ...
        <a href="/products" class="inline-block bg-gray-900 text-white px-6 py-2 rounded-xl hover:bg-gray-700 transition-colors">
          Browse Products
        </a>
      </div>
    )
  ...
  return (
    <div class="space-y-4">
      {items.map(item => (
        <div key={item.id} class="flex items-center gap-4 bg-white rounded-xl border border-gray-200 p-4">
          {item.imageUrl && (
      ...
      <div class="bg-white rounded-xl border border-gray-200 p-5 mt-4">
        <div class="flex justify-between items-center mb-4">
          ...
          <span class="text-xl font-bold text-gray-900">Rp {total.toLocaleString("id-ID")}</span>
        </div>
        <a href="/checkout" class="block w-full text-center bg-gray-900 text-white font-semibold py-3 rounded-xl hover:bg-gray-700 transition-colors">
          Proceed to Checkout
        </a>
      </div>
    </div>
