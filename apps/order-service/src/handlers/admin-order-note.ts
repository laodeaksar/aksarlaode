...
export const adminOrderNoteHandler = async ({ params, body, headers, set }: Context) => {
  // ── Authorization ─────────────────────────────────────────────────────────
  ...
  const adminId        = (headers["x-user-id"] as string | undefined) ?? "admin:unknown"
  ...
  if (trimmed.length > 1000) {
    set.status = 422
    return { error: "Note must not exceed 1000 characters", code: "NOTE_TOO_LONG" }
  }
