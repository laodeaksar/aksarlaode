...
const authHeader = (): string => `Basic ${btoa(env.MIDTRANS_SERVER_KEY + ":")}`
...
function toMidtransError(e: unknown): MidtransError {
  ...
  const rawMsg =
    typeof raw?.message === "string" ? raw.message : "Midtrans request failed"
  ...
  Effect.tryPromise({
    try: async () => {
      ...
      if (!res.ok) {
        ...
        throw {
          code: errBody.status_code ?? res.status,
          message: errBody.status_message ?? res.statusText,
        }
      }
