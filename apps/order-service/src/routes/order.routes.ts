...
export const orderRoutes = new Elysia({ prefix: "/orders", tags: ["Orders"] })
  ...
  .post("/:orderId/cancel", cancelHandler, {
    ...
    detail: {
      ...
      description: [
        ...
        "Returns the full updated order (shaped — no Mongoose internals, no admin notes).",
        "cancelledAt is guaranteed to be set in the response when status is CANCELLED.",
        "Returns 409 if the order is already PROCESSING, SHIPPED, DELIVERED, or REFUNDED.",
      ].join(" "),
    },
  })
  ...
  .patch("/:orderId/status", updateStatusHandler, {
    ...
    detail: {
      ...
      description: [
        "Admin-only or internal service status transition.",
        "Actor (changedBy) is recorded in statusHistory.",
        "Returns the full updated order (shaped — no Mongoose internals, no admin notes).",
        "Internal callers must supply x-service-token; admin callers must supply x-user-role: ADMIN.",
      ].join(" "),
    },
  })
