...
const create = (data: { token: string; userId: string; expiresAt: Date }) =>
  Effect.tryPromise({
    try:   () => db.insert(schema.passwordResetTokens).values(data).returning().then(r => r[0]!),
    catch: (e) => new DbError({ cause: e }),
  })

const findByToken = (token: string) =>
  Effect.tryPromise({
    try:   () => db.select().from(schema.passwordResetTokens)
...
const deleteByToken = (token: string) =>
  Effect.tryPromise({
    try:   () => db.delete(schema.passwordResetTokens)
...
export const resetTokenRepository = { create, findByToken, deleteByToken, deleteAllByUserId }
